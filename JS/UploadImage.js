$(document).ready( function() {
  ...
  $(document).ready(function(){
    ...
    window.genInput = function(ph,baseName,type,val,extraClass) {
        return '<input class="input form-control '+extraClass+'" name="'+baseName+'[]" type="'+type+'" value="'+val+'" placeholder="'+ph+'">';
    }

    window.genBtn = function(label,extraClass) {
        return '<button class="btn '+extraClass+'" id="b2" >'+label+'</button>';
    }

    window.genNewRow = function(type,val) {
        switch(type) {
            default:
                return '<div class="row record"><div class="col-sm-10">'+genInput('Tel.','field','text',val)+''+genBtn('-', 'btn-danger remove-parent')+'</div></div>';
                /*return '<div class="row record"><div class="col-sm-10">'+genInput('Tel.','field','text',val)+'</div><div class="col-sm-2">'+genBtn('-','btn-danger remove-parent')+'</div></div>';*/
        }
